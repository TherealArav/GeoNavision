...
from storage import QueryStorage, QueryRecord
...
class GoogleMapsPOIRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str) -> list[Document]:
        ...
        for poi in places:
            ...
            wheelchair_entrance: bool = acc.get(
                "wheelchairAccessibleEntrance", "Unknown"
            )
            wheelchair_restroom: bool = acc.get(
...
def is_rate_limit() -> bool:
    ...
    if current_time - last_time < COOLDOWN_TIME:
        time_left = int(
            COOLDOWN_TIME - (current_time - st.session_state.last_click_time)
        )
...
def get_rag_response(
    _query: str, _lat: str, _lon: str, _keys: dict[str, str]
...
"""
    ...
    if not docs:
        return (
            "I couldn't find any specific places matching your search in this area.",
            [],
        )

    formatted_context = "\n".join(
        [
            f"{d.metadata['poi_name']} ({d.metadata['distance_km']}km): {d.page_content}"
            for d in docs
        ]
    )
...
if "user_lat" not in st.session_state:
    st.session_state.user_lat = 25.1018
if "user_lon" not in st.session_state:
    st.session_state.user_lon = 55.1628
...
if st.session_state.auth:
    ...
    if c3.button("Run Exploration"):
        ...
        if not utilities.check_user_cords(
            st.session_state.user_lat, st.session_state.user_lon
        ):
            st.error(
                "Invalid cordinates. Latitude must be between -90 and 90, Longitude must be between -180 and 180."
            )
            st.stop()

        if not utilities.check_user_query(query):
            st.error(
                "Invalid query. Please enter a valid search term (alphanumeric and spaces only, max 100 characters)"
            )
            st.stop()
        ...
        st.session_state.cache = connection.find_nearby_query(
            query_text=query,
        ...
        if cache_result:
            ...
            try:
                if not all(keys.values()):
                    raise Exception(
                        "One or more API keys are missing. Please check your environment variables."
                    )

                st.session_state.summary, st.session_state.docs = get_rag_response(
                    query, st.session_state.user_lat, st.session_state.user_lon, keys
                )
    ...
    if c4.button("Clear Cache") and st.session_state.cache:
        ...
        st.info(
            "Cache exists for this query and location. Click 'Clear Cache' to remove it."
        )
    else:
